...
sequenceDiagram: Start RMI Connection
...
sequenceDiagram
    ...
    Cn-->>D: getMatches(client)
    Note right of D: new RMISender(client), creates playerID and sets player's state in MAIN_MENU
    ...
    alt CreateMatch
        ...
        G->>D: createMatchResult(matchID, nickname)
        D-->>Cn: createMatchResult(matchID, nickname)
        ...
        alt Success
        M->>G : 
        Note left of G: Server: changes player's state in MATCH_LOBBY

        G->>D: selectMatchResult(matchID, nicknames)
        D->>Cn: selectMatchResult(matchID, nicknames)

        alt ChooseNickname
        Note left of Cn: Update player's state in: CHOOSING_NICKNAME and update view
        ...
        D->>G: chooseNickname(playerID, nickname, matchController)
        ...
        alt Wait for other players to join
            ...
            D-->>Cn : chooseNicknameResult(nickname)
            Note left of Cn: Update player's state in: MATCH_LOBBY
            Note left of Cn: "Waiting for players to join..."
        else GameStarts
            M->>G: 
            G->>M: checkForGameStart()
            ...
            M->>G: true
            G->>D: sendGameStart(nickname, IDtoOpponentNickname, resDeckCardID, visibleResCardID1, visibleResCardID2, goldDeckCardID, visibleGoldCardID1, visibleGoldCardID2, starterCardID)
...
```
sequenceDiagram: Start Socket Connection
...
sequenceDiagram
    ...
    Note right of G: Server: Creates SocketSender and PlayerID
...
sequenceDiagram
    ...
    Note right of G: Server: Game start
    ...
    Cn->>V: startGame(nickname, IDtoOpponentNickname, resDeckCardID, visibleResCardID1, visibleResCardID2, goldDeckCardID, visibleGoldCardID1, visibleGoldCardID2, starterCardID)
    ...
    V->>Cn : chooseSecretObjective(cardID)
...
sequenceDiagram
    ...
    V->>Cn : placeCard(cardID, side, pos)
    Cn-->>Sn : placeCard(playerID, cardID, side, pos)
    Sn->>M: placeCard(playerID, cardID, pos, chosenSide)
    ...
    Cn->>V: placeCard(playerId, cardID, pos, side, deltaScore)
    Note left of V: Update current player's state in: DRAWING and update and print actual score of the current player for all the players
    Note left of V: Prints new playArea and drawArea for the current player
    Note left of V: Prints available commands for the current player : DrawResourceDeck, DrawVisibleResourceCard, DrawGoldDeck, DrawVisibleGoldCard
    ...
    V->>Cn : drawResourceCard() / drawGoldenCard()
    ...
    V->>Cn : drawVisibleResourceCard(index) / drawVisibleGoldenCard(index)
...
```
sequenceDiagram: Extra round turn 
...
sequenceDiagram
    ...
    Note right of M: Server: broadcast the ranking
