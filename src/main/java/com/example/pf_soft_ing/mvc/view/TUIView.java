...
public class TUIView implements View {
    ...
    private void interpretInput(String userInput) {
        ...
        if (legalCommands.contains(command)) {
            switch (command) {
                ...
                case "go" -> { // GetObjectives
                    if (parts.length != 1) {
                        System.out.println("\nError: GetObjectives does not take any arguments. Please, try again");
                        break;
                    }
    ...
    private void printObjectives() {
        ...
        System.out.println("\nSecret objective:");
        cardIDToCardFrontTUILines.get(secretObjectiveCardID1).forEach(System.out::println);
    ...
    public void showFirstPlayerTurn(int lastPlayerID, int playerID, int[] playerIDs,
        ...
        // Shows the required information without confirming the secret objective
        showFirstPlayerTurnAfterRec(playerID, playerIDs, starterCardIDs, starterCardSides, tokenColors, playerHands);
    }
    ...
    public void showFirstPlayerTurnAfterRec(int playerID, int[] playerIDs,
        ...
        System.out.println("""
                ...
                To view the objectives, type: go""");
    ...
    private void createStateToCommandsMap() {
        ...
        stateToCommands.put(PlayerState.COMPLETED_SETUP,
                new ArrayList<>(List.of("chat", "gmc", "gs", "opa", "gmi", "pa", "go")));

        stateToCommands.put(PlayerState.WAITING,
                new ArrayList<>(List.of("fc", "gh", "chat", "gmc", "gs", "opa", "gmi", "pa", "go")));

        stateToCommands.put(PlayerState.PLACING,
                new ArrayList<>(Arrays.asList("fc", "gh", "pc", "chat", "gmc", "gs", "opa", "gmi", "pa", "go")));

        stateToCommands.put(PlayerState.DRAWING,
                new ArrayList<>(Arrays.asList("fc", "gh", "ddr", "dvr", "ddg", "dvg", "chat", "gmc", "gs", "opa", "gmi", "pa", "go")));
